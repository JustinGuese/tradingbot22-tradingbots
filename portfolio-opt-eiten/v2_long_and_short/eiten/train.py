...
# in this version we support shorting
# next percentualize the weights (to sum up to 1)

for weight in WEIGHTS:
    total = [*WEIGHTS[weight].values()]
    total = sum([abs(x) for x in total])
    if total != 1:
        for ticker in WEIGHTS[weight]:
            WEIGHTS[weight][ticker] = WEIGHTS[weight][ticker] / total
        total = [*WEIGHTS[weight].values()]
        total = sum([abs(x) for x in total])
        if not (1.01 > total > 0.99):
            raise ValueError("Weights do not sum up to 1.0 : ", total)
